...
function generateRandomCocktail() {
    const cocktailNames = [
        'Margarita', 'Martini', 'Mai Tai', 'Cosmopolitan', 'Old Fashioned', 'Mojito', 'Piña Colada', 'Whiskey Sour', 'Tom Collins'
    ];

    // Get a random cocktail name from the array
    const randomIndex = Math.floor(Math.random() * cocktailNames.length);
    return cocktailNames[randomIndex];
}
